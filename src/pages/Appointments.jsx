...
import { getMyAppointments, reset, createFreeSlot, getAvailableSlots, bookAvailableSlot, updateAppointmentStatus } from '../store/slices/appointmentSlice';
...
const Appointments = () => {
    ...
    const filteredAppointments = appointments.filter(app => {
        ...
        return activeTab === 'upcoming' ? !isPast && app.status !== 'completed' : isPast || app.status === 'completed';
    });
        ...
        <div className="space-y-8 pb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <AnimatePresence mode="popLayout">
                    {(activeTab === 'slots' ? (user?.role === 'teacher' ? appointments.filter(a => a.status === 'available') : availableSlots) : filteredAppointments).length > 0 ? (
                        (activeTab === 'slots' ? (user?.role === 'teacher' ? appointments.filter(a => a.status === 'available') : availableSlots) : filteredAppointments).map((app) => (
                            <motion.div
                                key={app._id}
                                layout
                            ...
                            >
                                ...
                                <div className="flex items-center gap-4 mb-8">
                                    <div className="w-14 h-14 rounded-2xl bg-secondary-50 border border-secondary-100 flex items-center justify-center text-xl font-bold text-secondary-400 group-hover:bg-primary-600 group-hover:text-white transition-all duration-300 shadow-inner uppercase">
                                        {(user?.role === 'student' ? app.teacher?.name : (app.student?.name || 'F'))?.charAt(0)}
                                    </div>
                                    <div className="overflow-hidden">
                                        <h3 className="font-bold text-secondary-900 group-hover:text-primary-600 transition-colors truncate uppercase tracking-tight">
                                            {user?.role === 'student' ? app.teacher?.name : (app.student?.name || 'Available Free Slot')}
                                        </h3>
                        ...
                        <div className="col-span-full py-24 text-center bg-white border border-secondary-100 rounded-3xl shadow-sm flex flex-col items-center transition-colors">
                            ...
                            <p className="text-secondary-500 mt-2 max-w-sm mx-auto font-medium">There are currently no {activeTab === 'slots' ? 'open booking windows' : 'scheduled sessions'} matching your profile.</p>
                        </div>
                    )}
                </AnimatePresence>
            </div>
